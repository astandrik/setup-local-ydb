...
exports.sanitizeDockerName = sanitizeDockerName;
const node_path_1 = __nccwpck_require__(6760);
const node_os_1 = __nccwpck_require__(8161);
exports.IMAGE_REPOSITORY = "ghcr.io/ydb-platform/local-ydb";
exports.DEFAULT_VERSION = "26.1.1.6";
exports.DEFAULT_TENANT = "/local/test";
exports.DEFAULT_ROOT_DATABASE = "/local";
...
function buildRuntimeConfig(inputs, ports, resolvedVersion, env = process.env) {
    ...
    return {
        ...
        image: `${exports.IMAGE_REPOSITORY}:${resolvedVersion}`,
        auth: inputs.auth,
        ...
        dynamicNodeAuthTokenFile: (0, node_path_1.join)(authRoot, "dynamic-node-auth.pb"),
...
function defaultContainerPrefix(env) {
    const parts = [
        "setup-local-ydb",
        env.GITHUB_RUN_ID,
...
function findDuplicatePorts(ports) {
    const seen = new Set();
...
function writeFileCommand(envName, name, value) {
    const filePath = process.env[envName];
    if (!filePath) {
        issueCommand(envName.toLowerCase().replace("github_", ""), `${name}=${value}`);
        return;
    }
    const delimiter = `setup_local_ydb_${(0, node_crypto_1.randomUUID)()}`;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.CommandRunner = exports.CommandError = void 0;
exports.shellQuote = shellQuote;
exports.redact = redact;
exports.resultOutput = resultOutput;
exports.assertCommandOk = assertCommandOk;
...
const exec_1 = __nccwpck_require__(8592);
const ports_1 = __nccwpck_require__(3473);
const state_1 = __nccwpck_require__(696);
const version_1 = __nccwpck_require__(9629);
const ydb_1 = __nccwpck_require__(3016);
async function run() {
    const runner = new exec_1.CommandRunner();
    let runtimeConfig;
    try {
        const inputs = (0, config_1.parseActionInputs)(core.getInput, process.env);
        const resolvedVersion = await (0, version_1.resolveLocalYdbVersion)(inputs.version);
        ...
        if (runtimeConfig) {
            await (0, ydb_1.collectDiagnostics)(runtimeConfig, runner).catch((diagnosticError) => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.resolveRuntimePorts = resolveRuntimePorts;
const node_net_1 = __nccwpck_require__(7030);
async function resolveRuntimePorts(inputs) {
    const used = new Set();
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.resolveLocalYdbVersion = resolveLocalYdbVersion;
exports.normalizeVersionInput = normalizeVersionInput;
exports.compareNumericTagsDescending = compareNumericTagsDescending;
const config_1 = __nccwpck_require__(5751);
async function resolveLocalYdbVersion(versionInput, fetchImpl = fetch) {
...
function normalizeVersionInput(versionInput) {
    const trimmed = versionInput.trim() || config_1.DEFAULT_VERSION;
    const imagePrefix = `${config_1.IMAGE_REPOSITORY}:`;
    if (trimmed.startsWith(imagePrefix)) {
        return trimmed.slice(imagePrefix.length);
    }
    if (trimmed.includes("/") || trimmed.includes(":")) {
        throw new Error(`version must be a local-ydb tag, latest, or ${config_1.IMAGE_REPOSITORY}:<tag>`);
    }
    return trimmed;
}
function compareNumericTagsDescending(left, right) {
...
async function fetchLocalYdbTags(fetchImpl) {
...
async function fetchBearerToken(response, fetchImpl) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.setupLocalYdb = setupLocalYdb;
exports.cleanupLocalYdb = cleanupLocalYdb;
...
async function ensureTenant(config, runner, authenticated) {
    for (let attempt = 1; attempt <= STATUS_ATTEMPTS; attempt += 1) {
        ...
        const output = (0, exec_1.resultOutput)(status);
        ...
        if (/Unknown tenant|NOT_FOUND/i.test(output)) {
            ...
            if (!create.ok && !/Group fit error|failed to allocate group|no group options/i.test((0, exec_1.resultOutput)(create))) {
                throw new exec_1.CommandError(create);
            }
        }
...
async function prepareAuthArtifacts(config, runner) {
    ...
    const document = yaml_1.default.parse(generatedConfig.stdout);
    ...
    await (0, promises_1.writeFile)(config.authConfigPath, yaml_1.default.stringify(document, { aliasDuplicateObjects: false }), { mode: 0o600 });
    await (0, promises_1.writeFile)(config.rootPasswordFile, `${rootPassword}\n`, { mode: 0o600 });
...
async function readGeneratedConfigPath(config, runner) {
...
function generatedConfigDiscoveryLines(variableName) {
    return [
        `${variableName}=`,
...
async function runYdbd(config, runner, args, options = {}) {
...
async function runYdbdWithPassword(config, runner, args, options = {}) {
    const password = await readPasswordFile(config);
    const innerCommand = `/ydbd --server localhost:${config.ports.staticGrpc} --user ${(0, exec_1.shellQuote)(config.rootUser)} --password-file "$password_file" ${args.map(exec_1.shellQuote).join(" ")}`;
...
async function printDiagnosticCommand(runner, command, args) {
...
function extractRootPassword(securityConfig) {
    ...
    const root = users.find((user) => Boolean(user) && typeof user === "object" && !Array.isArray(user) && user.name === "root");
...
async function readPasswordFile(config) {
    const { readFile } = await Promise.resolve().then(() => __importStar(__nccwpck_require__(1455)));
